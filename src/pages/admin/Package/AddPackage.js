...
const AddPackage = () => {
  ...
  const addPackage = () => {
    if (packageData.packageBaseRate < calculateTotalCharges()) {
      toast.error("Base rate should be more than " + calculateTotalCharges());
      return;
    }
    ...
    insertPackage(packageData)
      .then((resp) => {
        console.log("Package " + resp);

        //Upload Image
        uploadImages(resp.packageId, formData)
          .then((resp) => {
            console.log("Package Added successfully");
            toast.success("Package Added successfully");
            console.log("Package Image Path :" + resp);
            resetPackageData();
  ...
  const calculateTotalCharges = () => {
    let total = 0;
    total += packageData.room.roomRate;
    packageData.pkgActivity.map((act) => {
      total += act.activities.activityRate;
    });
    return total;
  };
  ...
  const handleCityChange = (event, property) => {
    if (event.target.value === "NA") {
      ...
      getHotelsByDestCity(event?.target?.value)
        .then((resp) => {
          console.log(resp);
          setHotels(resp);
      ...
      getActivitiesByDestCity(event?.target?.value)
        .then((resp) => {
          console.log(resp);
          setActivities(resp);
          //Reset previously selected data
  ...
  const handleHotelChange = (event, property) => {
    if (event.target.value === "NA") {
      ...
      // Logic to set minimum room
      // const minRateRoom = packageData?.hotel?.rooms.reduce(
      //   (prevRoom, currentRoom) =>
      //     (prevRoom?.roomRate ?? currentRoom?.roomRate) >= currentRoom?.roomRate
      //       ? currentRoom
      //       : prevRoom,
      //   null
      // );
      // console.log("****" + minRateRoom);
    }
  ...
  const handlePackageImageChange = (e) => {
    ...
    if (e.target.files[0] !== undefined) {
      document
        .getElementById("pkgImg")
        .setAttribute("src", URL.createObjectURL(e.target.files[0]));
    } else {
      document.getElementById("pkgImg").setAttribute("src", "");
    }
  };
              ...
              <CardBody>
                <Form>
                  ...
                  <Row>
                    ...
                    <Col md={4}>
                      <FormGroup>
                        ...
                        <FormText className="ms-3">Choose Image</FormText>
                        <Input
                          id="packageImage"
                          name="packageImage"
                          type="file"
                  ...
                  <Row>
                    <Col md={6}>
                      <FormGroup>
                        ...
                        />
                        Per Head (Rate Incl. of Hotel Charges, Activity
                        charges,Pick up Drop from Airport)
                        <br />
                        Base rate should be more than :: ₹
                        {isNaN(calculateTotalCharges())
                          ? 0
                  ...
                  <Row className="text-center mt-3">
                    <Col md={12}>
                      <div>
                        <Button color="primary" onClick={addPackage}>
                          Add Packages
                        </Button>
                        <Button
                          className="ms-5"
